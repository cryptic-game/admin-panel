...
export class TeamService {
  ...
  public addMember(
    ...
    this.apiService
    .endpoint<TeamMember>('website/team/member/add', {
      name,
      github_id: githubId,
  ...
  public updateMember(member: TeamMember): void {
    this.apiService
    .endpoint<TeamMember>('website/team/member/update', {
      id: member.id,
      name: member.name,
    ...
    .subscribe((m) => {
      ...
      updateMember.joined = Math.floor(
        new Date(m.body.joined).getTime() / 1000
      );
    });
  }
  ...
  private updateCache(): void {
    forkJoin({
      departments: this.apiService.endpoint<TeamDepartment[]>(
        'website/team/department/list'
      ),
      members: this.apiService.endpoint<TeamMember[]>(
        'website/team/member/list'
      ),
    }).subscribe(
      (data) => {
